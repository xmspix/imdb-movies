...
const Movie = ({ match: { params } }: any) => {
  ...
  const movie: any = store.movies
    .filter(movie => movie.title?.toLowerCase().match(/[a-z0-9]+/gi).join('-') === params.title)[0]
  ...
  const isCollected = store.collection.filter(
        ...
        <div className="movie-container__header--info">
          <div className="movie-container__header--info-title">
            ...
            <svg
              ...
              onClick={() => {
                return !isCollected ? store.addToCollection(movie) : null;
